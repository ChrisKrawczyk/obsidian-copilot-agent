...
5. **Create the annotated tag.** Extract the CHANGELOG body for `<version>` via `npm run --silent changelog:extract -- <version>` and pipe to `git tag --annotate v<version> --file -`. Use `--silent` — without it, npm prepends banner lines (`> obsidian-copilot-agent@<version> changelog:extract` and the resolved script command) that would land in the tag message and cause `verify` to flag a body mismatch. The annotated tag's message carries the release notes verbatim — this is the source of truth that the `release.yml` workflow re-extracts later for the GitHub Release body.
...
- **`gh release create --target` requires a FULL 40-character SHA.** Short SHAs return `HTTP 422 Release.target_commitish is invalid`. This skill's standard path doesn't hit it (it uses `git tag` then a tag-triggered workflow, not `gh release create --target`), but the `bootstrap-v0.5.0.mjs` script DOES hit it and resolves the short SHA via `git rev-parse <sha>` upfront. If you ever extend this skill to publish via `gh release create` directly (e.g. a manual fallback path), pass the full SHA.
- **On Windows, `execFileSync` cannot spawn `.cmd` shims (`npm.cmd`, `npx.cmd`) without `shell: true`.** Standard pattern: `shell: process.platform === "win32"`. This applies to ad-hoc Node scripts that shell out to npm/npx (the release agent's primary path uses `npm run` directly via the maintainer's shell, so it's not affected, but any helper script invoked from the agent should set `shell: true` on Windows).
- **`gh auth switch` can flip back unexpectedly across processes.** On a multi-account host, run `gh auth status | Select-String "Active account: true"` immediately before any `gh` mutation and re-switch if the wrong account is active. This is documented in `RELEASING.md` under "Two-`gh`-account note".

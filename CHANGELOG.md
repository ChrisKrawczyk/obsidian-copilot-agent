...
- Schema versioning of persisted state with corruption-recovery sidecar: a malformed `data.json` is preserved verbatim at `<plugin-dir>/conversations_recovery.bak.json` and the plugin proceeds from defaults; auth and safety settings survive recovery (FR-010, SC-001).

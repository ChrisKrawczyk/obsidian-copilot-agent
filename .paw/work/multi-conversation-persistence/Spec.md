...
- Plugin loads on a v0.2 install (no persisted conversation data exists yet): a single empty "Untitled" conversation is created. v0.2's in-memory message history is not recoverable across the plugin reload that ships v0.3 (see FR-019 and Assumptions).
...
- **FR-010**: If persisted data fails to parse on load (corruption, schema mismatch), the plugin starts with a fresh empty conversation, surfaces a non-blocking Notice describing the recovery, and writes the malformed conversations-store subtree (the keys owned by the conversations store: `conversations`, `activeConversationId`, and the conversations-store `schemaVersion`) to a sibling backup file named `conversations_recovery.bak.json` in the same plugin-data directory before overwriting only those keys with defaults. The shared `data.json` file is NOT renamed, so other top-level keys owned by sibling stores (`auth`, `safety`) remain intact and functional. (Stories: P2)
...
- **FR-014**: A new plugin setting "Expose v0.1 raw-filesystem tools" (default ON) gates whether the six v0.1 raw-FS tools (`view`, `read_file`, `search_content`, `create_file`, `edit_file`, `delete_file`) are offered to the model. When ON (default), the tools are offered alongside the v0.2 vault tools as a FALLBACK, with the preamble directing the model to prefer vault-aware tools first; this preserves the v0.1 escape hatch for non-markdown files and operations the vault-aware tools don't cover. When OFF (user opt-out), the model is not offered these tools and cannot invoke them, yielding a strictly vault-only agent. (Stories: P4)
...
- **SC-004**: With the advanced setting OFF (user opt-out), 0 of the 6 gated raw-FS tools appear in the tools manifest exposed to the model in a fresh session. With the setting ON (default), all 6 are present AND the preamble's tool inventory marks them as fallbacks, instructing the model to prefer vault-aware tools first. (FR-014, FR-015)

...
- **FR-010**: If persisted data fails to parse on load (corruption, schema mismatch), the plugin starts with a fresh empty conversation, surfaces a non-blocking Notice describing the recovery, and renames the corrupted file with a `.bak` suffix to preserve the data for manual inspection. (Stories: P2)
...
- **FR-014**: A new plugin setting "Expose v0.1 raw-filesystem tools" (default OFF) gates the registration of the six v0.1 raw-FS tools (`view`, `read_file`, `search_content`, `create_file`, `edit_file`, `delete_file`). When OFF, these tools are not registered with the SDK session and the model has no way to invoke them. When ON, they are registered with their existing v0.1 behavior and approval policy. (Stories: P4)
...
- **FR-017**: The agent gains three new read-only vault tools, all auto-approved under the v0.2 FR-017 read-only gate:
  - `search_by_tag(tag: string)` — returns the list of notes whose metadata cache reports the given tag (with or without `#` prefix). Output: array of `{ path, basename }`. Result count is capped at 200; if exceeded, results are truncated with a `truncated: true` flag in the response.
  - `search_by_name(query: string)` — returns notes whose basename contains the query as a case-insensitive substring, ranked by closeness of match (exact match > prefix match > substring match). Output: array of `{ path, basename }`, capped at 50.
  - `list_all_tags()` — returns every distinct tag observed in the vault's metadata cache. Output: array of `{ tag, noteCount }`, sorted by `noteCount` descending. (Stories: P5)
...
- **FR-019**: First-time migration from v0.2: when the plugin loads with no persisted conversation data, any in-memory conversation state from a previously running v0.2 session in the same Obsidian process is migrated into a single conversation named per FR-005. If there is no in-memory state (clean reload), a single empty "Untitled" conversation is created. (Stories: P2)
...
- **FR-021**: All v0.2 invariants are preserved: universal permission gate, FR-017 read-only auto-approval, deny-by-default tool execution, no regressions to streaming, Stop control, approval-prompt flow, token rotation, the 13 v0.2 vault tools, or the 410/410 baseline test count.
...
- **PersistedState**: schemaVersion (integer), activeConversationId (string), conversations (array of Conversation), settings (existing settings shape extended with the new flags).
...
- **SC-005**: The agent answers "list all my notes tagged X" by invoking `search_by_tag` (verified via tool-call telemetry) without prompting the user for approval, and returns exactly the matching notes with no false positives or negatives. (FR-017, FR-018)
...
- **SC-007**: After deliberately corrupting the persisted data file and reloading the plugin, the user sees a recovery Notice, the plugin opens with a fresh empty conversation, and the corrupted file is preserved as `<original>.bak` on disk. (FR-010)
- **SC-008**: The 21st conversation creation archives the oldest non-active conversation: it disappears from the picker but its data file is retained, and the active conversation count in the picker stays at 20. (FR-002)
...
- Persisted conversation state lives in `data.json` via Obsidian's standard `Plugin.saveData()` / `Plugin.loadData()` API. This is shared with existing settings and is the standard pattern for Obsidian plugins.
...
- Cross-restart undo's "file modified since snapshot" detection uses file mtime + size as a heuristic, with content-hash verification only when the heuristic is ambiguous. This is consistent with how the in-session UndoJournal already detects external edits.
...
- "Active conversation when Obsidian closed" is captured at the latest `onunload` opportunity (or via a debounced `lastActiveAt` write whenever the user switches). Either approach satisfies FR-009.
...
- Migration from a v0.2 in-memory session captures only the messages currently visible in the chat pane; deeper history (which v0.2 didn't keep persistently anyway) is not recoverable.
...
- **Persisted-state corruption silently breaks every load**: Impact: data loss for all conversations. Mitigation: schema versioning; FR-010 corruption-recovery path with `.bak` preservation; aggressive automated tests for malformed inputs.
...
- **Persisted-state file grows unbounded for power users**: Impact: slow load times, large `data.json`. Mitigation: SC-011 size budget surfaces as a non-blocking Notice; archived conversations counted toward total but not loaded eagerly into memory.

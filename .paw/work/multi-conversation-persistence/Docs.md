...
- Recovery path: when the persisted blob fails schema validation, the malformed conversation subtree is captured into a wrapped sidecar (`{ recoveredAt, schemaVersionExpected, malformed }`) at `<pluginDataDir>/conversations_recovery.bak.json` and the plugin proceeds with default conversation state. Auth/safety/settings keys survive recovery (FR-010).

### Migration Policy

The store carries a `schemaVersion` integer. v0.3 ships at `schemaVersion: 1`. On load, `ConversationsStore.load()` first checks whether the persisted blob has any of our keys (`schemaVersion`, `conversations`, `activeConversationId`) — if NONE are present (e.g. a fresh install or a v0.2-only `data.json` that only carried `auth` / `safety`), the store starts from defaults and does not trigger recovery. When at least one of our keys IS present, `migrate()` runs:

- `schemaVersion === CURRENT_SCHEMA_VERSION` AND the rest of the subtree validates → loaded as-is.
- `schemaVersion` missing or different from CURRENT, OR a field-level validation failure → corruption-recovery path runs: the malformed subtree is wrapped into `{ recoveredAt, schemaVersionExpected, malformed }` and written to `<pluginDataDir>/conversations_recovery.bak.json`, then v0.3 starts from defaults. Sibling top-level keys (`auth`, `safety`, `settings`) are preserved.
- `schemaVersion > CURRENT_SCHEMA_VERSION` (forward incompatibility from a downgrade) hits the same recovery path so the future blob isn't truncated.

...
- **`src/settings/SafetySettingsStore.ts`**: Extend `SafetySettings` with `exposeRawFsTools: boolean` (default `true` — the v0.1 raw-FS tools are offered as a FALLBACK to the vault-aware tools, with the preamble directing the model to prefer vault tools first; users opt OUT for a strictly vault-only agent); apply the same field-by-field defaulting in `mergeWithDefaults` that `defaultMode`/`allowlist` use; preserve unknown top-level keys on write as today (`src/settings/SafetySettingsStore.ts:147-169`).
...
- **`src/domain/PreambleAssembler.ts`**: Accept the same filtered tool-entries list (or a `excludeRawFs` flag) so the preamble's tool inventory matches the SDK manifest when gating is on (`src/domain/PreambleAssembler.ts:114-115`). When the raw-FS tools ARE included (default), the inventory header marks them as fallbacks and tells the model to prefer vault-aware tools first; this is what makes the default-on posture safe by routing the model toward Obsidian-aware behavior without amputating the v0.1 escape hatch.
- **Tests**:

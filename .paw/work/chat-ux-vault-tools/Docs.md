...
- **Vault Awareness Settings.** Settings → Copilot Agent → Vault Awareness. Modes: **Default** (vault root + folders + tool inventory + conventions), **Custom** (user-supplied body, with `{{vaultRoot}}`, `{{today}}`, `{{timezone}}` placeholders), **None** (preamble suppressed entirely — for sensitive vaults). Changes apply on next session start.

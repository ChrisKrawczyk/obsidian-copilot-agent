...
- FR-012: An insert-into-active-note capability shall be available with insertion modes append, prepend, or replace, with cursor-position behavior as specified in P3 acceptance scenario 2. The capability does NOT guard against read-only / preview-mode views — Phase 4 manual verification showed Obsidian's own editor surface accepts inserts in reading view and the user explicitly opted to keep the operation working in that case. **The unsaved-editor-conflict guard that applies to disk-surface writes (FR-010) does NOT apply to the active-editor path**: the editor buffer is the authoritative state for inserts directed at the active note, so there is no on-disk-vs-buffer conflict to guard against (the conflict the FR-010 guard prevents is the opposite direction — a disk write clobbering an open editor's unsaved buffer). When `insert_into_active_note` falls back to `edit_note` (no active editor), FR-010's guard does apply because that path writes to disk. (Stories: P3) <!-- Amended in final review: original FR-012 also required a `read_only` structured error when the active file was in preview mode; the guard was deliberately omitted in implementation and the spec is amended here so the two stay in sync. -->
...
- Vault-aware preamble size shall be bounded such that the assembled default preamble (vault root + timezone + today + tool-inventory + authoring-conventions) is under 8 KB regardless of vault size (no per-vault enumeration is included by design — see FR-006/FR-007). <!-- Bumped from 4 KB in final review to accommodate FR-006b/c authoring-conventions guidance added in review (ask-before-new-tags + GFM/Tasks-plugin fallback). The preamble remains constant per vault and is sent once per session, so the increase is one-time and bounded. -->

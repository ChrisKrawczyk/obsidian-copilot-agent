...
- **`src/mcp/credentials/CredentialResolver.ts`** (new): `CredentialResolver` class with injected deps (clock, `CommandRunner`, redacted logger). Public surface:
  - `resolve(serverId: string, credentials: ServerCredentials): Promise<{ authorization: string; expiresAt: number | null } | null>`
  ...
  Internal cache keyed by `serverId`. Returns cached entry when `now < expiresAt - refreshBuffer`. Enforces minimum 5s between re-resolves per server (rate-limit guard for missing/unparseable `expiresOn`). Static-bearer returns immediately with `expiresAt = null`. `none` returns `null`. `oauth-pkce` returns a structured "not implemented in this release; see docs" error that downstream code converts into a user-facing remediation hint.
- **Tests**:
...
- **`src/mcp/McpManager.ts`**:
  ...
  - `callTool` HTTP path: on a caught `McpHttpError` with `status === 401`, calls `resolver.invalidate(serverId)`, retries the call exactly once, and on a second 401 surfaces a chat-side error: `"Credentials rejected" + (wwwAuthenticate ? ": " + wwwAuthenticate : "")` plus a remediation hint computed from the credential variant via a pure formatter (Phase 5 ships the M365-specific `az login --tenant <id>` template; the manager calls the formatter so this is testable here).
...
- **Tests**:
  - `src/mcp/McpServerRuntime.httpFetch.test.ts`: every outbound fetch carries `Authorization` from the resolver callback; the value can change between calls within one runtime instance (cache invalidation case); session DELETE also carries it.
...
- **`src/settings/mcpServerFormLogic.ts`** (pure helpers — node-only testable):
  ...
  - Add pure helpers `buildCredentialStatusText(state): string` and `buildRemediationHint(variant, lastTenantId?, error): { text: string; copyable: string }` — drives FR-014 wording (M365 hint is `az login --tenant <id>` when a previous successful resolution captured a tenant id; bare `az login` otherwise).
...
- **Tests**:
  ...
  - `src/settings/mcpServerFormLogic.test.ts`: extend with credential validation cases per kind, command tokenization, remediation-hint formatting with and without `lastTenantId`, refresh-buffer bounds.

...
**Covers:** FR-004, FR-005, FR-010 (resolver scope), FR-015 (timeout contract surfaces here); SC-006 (logging redaction)
...
- **`src/mcp/credentials/CredentialResolver.ts`** (new): `CredentialResolver` class with injected deps (clock, `CommandRunner`, redacted logger). Public surface:
  ...
  - `invalidate(serverId: string): void` — called on 401 and on credential-config change. Clears the cached token but **retains the last known `tenantId`** for that server (a non-secret GUID) so failure-path remediation formatters can still render `--tenant <id>` after the cache has been invalidated. Cleared only by `clear(serverId)`.
  - `getLastKnownTenantId(serverId: string): string | null` — non-throwing accessor returning the last successfully captured tenant id for that server (or `null` if never captured). Used by the manager when composing post-401 remediation hints.
  - `clear(serverId: string): void` — called on server removal; clears both the cached token and the last-known tenant id.
  ...
  **Tenant-id capture (FR-014 input)**: After a successful command-based resolution, the resolver attempts to extract a non-secret `tenantId` value from the resolved JSON by reading a top-level `tenant` field (Azure CLI's `az account get-access-token` JSON output always includes this for the only shipped preset). If absent, `tenantId` is left as `null`; the resolver does NOT decode the JWT payload in this release (rationale: avoids operating on bearer-token bytes, keeps the security surface minimal, and is sufficient for the M365 preset where the `tenant` field is always present — see spec FR-014 fallback that uses bare `az login` when no prior tenant id is known). The extracted `tenantId` is cached alongside the token AND retained across `invalidate(serverId)` calls so post-401 remediation formatters can still render `--tenant <id>`. Tests assert that extraction failure leaves `tenantId = null` and that the captured value never appears prefixed/suffixed by token bytes in any log output (FR-010).
- **Tests**:
  ...
  - `src/mcp/credentials/CredentialResolver.test.ts` with a fake clock + fake `CommandRunner`. Cases: static-bearer pass-through; cache hit within refresh buffer; cache miss outside refresh buffer re-runs the command; missing expiry → opaque-once then 5s rate limit; `invalidate()` clears cache forcing next resolve; command failure surfaces structured error (exit code, redacted stderr capped at 200 chars); JSON parse failure surfaces structured error referencing field *names* only (FR-010); **token field not found at configured path** (spec edge case 3) → structured error whose user-visible message includes the literal string `"token field not found at path: <configured-path>"` AND the configured path string (this MUST be a distinct test from the JSON-parse-failure case so the spec's per-edge-case error contract cannot be collapsed); expiry field missing at configured path (spec edge case 4 fallback) → resolver still returns a token but `expiresAt = null`; **per-server cache isolation** (spec edge case 7) → two resolver entries with distinct `serverId` values but identical `credentials` (same command, same scope) maintain independent cache slots (test: invalidating one does not affect the other; one server's cache hit does not satisfy the other server's request); `oauth-pkce` surfaces "not implemented" structured error; **tenant-id extraction** — happy path from `tenant` field; absent `tenant` field → `tenantId = null`; cached `tenantId` survives subsequent cache hits; **tenant-id retention across `invalidate()`** — after a successful resolve captures a `tenantId`, calling `invalidate(serverId)` clears the token but `getLastKnownTenantId(serverId)` still returns the captured value; `clear(serverId)` clears both.
...
**Covers:** FR-003, FR-015 (spawn implementation)
...
- **`src/mcp/McpHttpError.ts`** (new — co-located with the fetch wrapper's concerns; `httpPolicy.ts` remains URL/redirect/host-class scoped and is NOT modified for error typing): The fetch wrapper today throws a sanitized string error on non-OK responses. Introduce a typed `McpHttpError extends Error` carrying `status` (number) and `wwwAuthenticate` (optional string, scheme/realm — the auth challenge is not a secret; embedded `Bearer <token>` forms are still redacted) while preserving the existing redacted message contract. Constructed inside the fetch wrapper's non-OK response branch (`createMcpHttpFetchWrapper` in `src/mcp/McpServerRuntime.ts:506-538`). All existing callers continue to receive the same redacted message; only typed callers (the manager's 401 retry path) inspect `status` / `wwwAuthenticate`.
...
- **`src/mcp/credentials/RemediationFormatter.ts`** (new — defined in Phase 4 so Phase 4's chat-side error path is self-contained; Phase 5 supplies the M365-specific overrides without changing this contract):
  - Exports `interface RemediationFormatter { format(ctx: { variant: ServerCredentials["kind"]; command: string | null; lastTenantId: string | null; error: { kind: "command-failed" | "unauthorized" | "denied" | "timeout"; detail?: string } }): { text: string; copyable: string } }`. The `command` field carries the configured command-based `command` string verbatim (or `null` for non-command-based variants); this is what lets a specialized formatter detect "is this the M365 preset / an `az`-rooted command?" vs. a custom command-based helper and produce a correct hint (Gemini's review caught that a variant-only signature would emit `az login` for ALL `command-based` failures, breaking the P2 custom-command user story).
  - Exports a default implementation `DefaultRemediationFormatter` returning generic-but-actionable text per `(variant, error.kind)` pair. For `command-based` + `unauthorized`, the default copyable hint is a generic "ensure your credential command is signed in and emits valid JSON" — the default formatter does NOT emit `az login` for arbitrary commands; that specialization belongs to `M365RemediationFormatter` in Phase 5, which inspects `command` (first argv token must equal `az` or end in `\az.cmd` / `/az`) before substituting the M365-aware hint.
  - `McpManager` accepts the formatter via constructor injection (defaulting to `DefaultRemediationFormatter` if no override supplied); Phase 5's `main.ts` wiring swaps in `M365RemediationFormatter` (composed with `DefaultRemediationFormatter` as fallback). When invoking the formatter, the manager populates `lastTenantId` from `resolver.getLastKnownTenantId(serverId)` (the resolver retains this across `invalidate()` per Phase 2) and `command` from the persisted server credentials (or `null` if the credential variant is not command-based).
- **`src/mcp/McpManager.ts`** (continued): New method `onCredentialConfigChanged(serverId: string): void` — called by the settings save path (Phase 5) whenever the persisted `credentials` block for a server differs from its prior value (including variant switches and command edits). It delegates to `resolver.invalidate(serverId)`. This is the wiring the spec's edge case 9 ("user changes credential command → cached token invalidated immediately") requires; without this hook, a user fixing a typo'd command would continue to hit the cached failure until plugin reload.
...
- **Tests**:
  ...
  - `src/mcp/McpHttpError.test.ts` (new): typed `McpHttpError` carries `status` and `wwwAuthenticate`; constructed from the fetch wrapper's non-OK branch; embedded `Bearer <token>` substrings inside `wwwAuthenticate` are redacted while scheme/realm tokens survive.
  - `src/mcp/httpPolicy.test.ts`: existing redirect / private-IP / metadata-host tests stay green (no behavior change in this module per S4').
  - `src/mcp/McpManager.test.ts`:
    ...
    - **`onCredentialConfigChanged` invalidates the cache** (spec edge case 9): calling `manager.onCredentialConfigChanged(serverId)` calls `resolver.invalidate(serverId)` exactly once for the matching server and not for unrelated servers; a subsequent `callTool` re-resolves the credential.
    ...
    - **Post-401 `lastTenantId` plumbing**: after a successful resolve captured `tenantId = "T1"` then a 401-driven invalidate, the formatter receives `lastTenantId = "T1"` (not `null`) because `resolver.getLastKnownTenantId(serverId)` retains it across `invalidate()`.
...
- **`src/mcp/credentials/M365RemediationFormatter.ts`** (new): Implements the `RemediationFormatter` interface defined in Phase 4. The formatter inspects the new `command` field on the context: it only specializes `(command-based, unauthorized)` when `command` is non-null AND its first argv token (after tokenization) is `az` OR its resolved basename matches `az.cmd` / `az.bat` / `az` (case-insensitive on Windows). When the predicate matches, returns `text = "Azure CLI credentials are not signed in or have expired."` and `copyable = lastTenantId ? "az login --tenant " + lastTenantId : "az login"`. For all other cases (non-`az` command-based servers, non-unauthorized errors, non-command-based variants) it delegates to a composed `DefaultRemediationFormatter`. This preserves the P2 user-story contract (custom command-based helpers do NOT get misleading `az login` hints).
...
- **`src/settings/McpServersSection.ts`** (DOM thin — delegates to the helpers above):
  ...
  - **Credential-config-change → cache invalidate** (spec edge case 9): the form save path (`saveForm` or equivalent) performs a structural comparison between the persisted `credentials` block of the prior server entry and the new value. If they differ (variant change OR any field change including the command string), call `manager.onCredentialConfigChanged(serverId)` exactly once before re-enabling the server. New-server adds also call this (idempotent on a server with no cached state). This is the wiring that satisfies spec edge case 9; the trust-epoch grant revocation path is unchanged (still keyed on `name + transport + url` per FR-011).
- **Tests**:
  ...
  - `src/settings/McpServersSection.test.ts`: extend with preset-dropdown population, preflight hint surfaced when `findOnPath` returns null, credential editor visibility per kind, Test-connection happy-path and failure messaging.
    - **Credential-config-change triggers invalidate** (spec edge case 9): editing only the credential command (variant unchanged) on a saved server calls `manager.onCredentialConfigChanged(serverId)` exactly once; editing only the display name does NOT; switching variants (e.g. `static-bearer` → `command-based`) also calls it exactly once.
    - **Toggle-retains-cache** (spec edge case 10): the server enable/disable toggle does NOT call `manager.onCredentialConfigChanged(serverId)` and does NOT call `resolver.invalidate(serverId)` — toggling off then back on preserves the cached credential. (Implemented by ensuring the toggle path bypasses the form-save path's credential-diff branch.)

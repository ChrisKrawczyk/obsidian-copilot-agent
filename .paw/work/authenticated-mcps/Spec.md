...
- FR-003: For `command-based` credentials, the plugin SHALL execute the configured command using a non-shell process spawn (`shell: false`; no joined shell command string; the user-supplied argv is never interpreted by a shell) and parse the resulting stdout as JSON. On Windows, when the resolved executable is a `.cmd` / `.bat` script, the plugin MAY invoke it through `cmd.exe /d /s /c <resolved-path> <argv>` with the resolved path and arguments passed as a single argv array (not as a joined command string) — this matches the existing stdio MCP transport's Windows path-resolution behavior and preserves the no-shell-interpolation property because the user-controlled argv tail is never concatenated into a shell command line. (Stories: P1, P2)
...
- SC-007: All existing tests continue to pass (baseline verified during code research: 1107 passing tests across 72 files, superseding older v0.5 docs that cited 944 / 970+); the existing stdio MCP servers (Foam, OneDrive) and any pre-existing unauthenticated HTTP server configurations continue to work with zero configuration changes. (FR-016)
...
- The command-execution path uses a non-shell process spawn (`shell: false`, argv as an array). The user types `az account get-access-token --scope ...`; the plugin tokenizes this into argv and spawns directly without composing a shell command line. On Windows, `.cmd` / `.bat` scripts are invoked via `cmd.exe /d /s /c <resolved-path> <argv>` (a single argv array, not a joined string) to preserve the existing stdio MCP transport's `.cmd` resolution behavior; the user-controlled argv tail is still never concatenated into a shell command line. (Rationale: avoids shell-injection class of bugs; matches how the plugin's existing stdio MCP transport already resolves CLI commands on Windows.)
...
- The plugin's existing settings persistence layer. (No encryption / OS-keychain helper exists today; static-bearer token values land on disk in plaintext per FR-002 and the cross-cutting risk below. Adding an encryption helper is tracked as future work and would not change the persisted shape.)

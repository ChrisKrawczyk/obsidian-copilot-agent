...
**Independent Test**: For v0.6.0, macOS/Linux are covered by automated `detectPlatformTuple()` unit tests for all six non-Windows tuples (per relaxed SC-003), with the corresponding platform-package fetch URLs verified statically against the npm registry. End-to-end BRAT install validation on macOS arm64 and Linux x64 is **deferred** beyond v0.6.0 — these platforms ship as alpha per R7 and the README's platform-support table, with users invited to file issues. The acceptance scenarios below remain the design contract these platforms must satisfy when manual smoke is added later, but they are NOT pre-release gates for v0.6.0.

**Acceptance Scenarios** (design contract — verified by tuple-detection unit tests for v0.6.0; deferred manual smoke):
...
- FR-020: When the binary is absent or does not match the pinned version, the plugin fetches the platform-correct `@github/copilot-<plat>-<arch>` npm package from the public npm registry, extracts the binary, and places it at the expected location. Pinned version is a constant baked into the plugin release at build time from the `@github/copilot` version resolved in `node_modules` at install time (which today is what `@github/copilot-sdk` transitively pulls in via its `peerDependencies`/`dependencies` — currently `1.0.59`). (Stories: P2, P3, P4)
...
- **The pinned binary version is the `@github/copilot` version resolved in `node_modules` at install time** (today this is what `@github/copilot-sdk` transitively pulls in via its dependency graph — currently `1.0.59`). This value is captured into the build at build time so the fetcher knows what to ask for. Runtime override of the pinned version is not supported in v0.6.0.

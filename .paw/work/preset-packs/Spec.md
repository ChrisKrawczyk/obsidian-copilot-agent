...
- **FR-021:** Pack diff and equality MUST be computed over a canonical form of the pack content: JSON key order normalized (lexicographic), whitespace collapsed, the persisted import metadata (timestamp, source path, internal record id) EXCLUDED from comparison. Two packs whose canonical forms are byte-equal are considered equal; otherwise the differing preset entries (by `id`) are surfaced as added / removed / changed in the diff. Top-level pack metadata changes (`label`, `version`) MAY additionally be surfaced separately from preset deltas in the re-import confirm dialog as a presentation aid; this does NOT constitute the deferred-to-v2 semantic diff. *(Stories: P3)*
...
- **SC-006:** Importing a pack JSON whose declared command does not exist on the current OS still succeeds; the install hint surfaces only when the user configures a server from one of its presets (existing form-level preflight behavior applies — pack-declared preflight behaves identically to built-in preflight per FR-015, which means the non-blocking hint renders the moment a `command` value is populated in the form, exactly as built-ins do today). *(FR-015)*

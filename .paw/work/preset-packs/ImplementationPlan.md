...
- **`src\settings\McpServersSection.ts`** (DOM layer):
  ...
  - Preflight hint rendering reused unchanged (`src\settings\McpServersSection.ts:416-427`). **Phase 4 introduces NO new preflight invocations and NO new code path that triggers `commandExists` / runtime probes. The existing form-level hint continues to fire only when the user has a `command` value in the form — exactly as today. The spec's SC-006 "when the user actually tries to use" wording is satisfied by leaving the existing runtime / tool-call path entirely unmodified; packs add nothing earlier (no probe on import, no probe on dropdown selection, no probe at form pre-fill).**

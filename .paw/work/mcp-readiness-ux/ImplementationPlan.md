...
- [x] **Phase 4 [deferred to future release]: Upstream SDK contribution** — External work. Filed as issue [github/copilot-sdk#1896](https://github.com/github/copilot-sdk/issues/1896) on 2026-07-02 (re-triage of closed #735). Blocked on maintainer response. Not blocking v0.9.0 — Phase 4.5 provides the user-visible outcome via a session-swap stop-gap. Follow-up captured in `CHANGELOG.md`, `README.md`, and `docs/mcp-readiness-ux.md`-adjacent references so it survives the `commit-and-clean` artifact stop-tracking at final PR time.
...
- [x] **Phase 5 [deferred to future release]: SDK adoption in plugin** — Blocked on Phase 4 landing and publishing to npm. When ready: bump `@github/copilot-sdk`, wire the real primitive in `applyToolListChange`'s branch 1, remove `swapSessionForToolRefresh` fallback. Small diff. Follow-up captured in the same user-facing docs as Phase 4.
- [x] **Phase 6: Documentation** — Docs.md created at `.paw/work/mcp-readiness-ux/Docs.md`; README `## What's new in v0.9` section added; CHANGELOG `[0.9.0]` entry added; version bumped to 0.9.0 in `package.json`, `manifest.json`, `versions.json`. Phase 4 and Phase 5 follow-ups captured in CHANGELOG "Follow-ups" and README "Forward-looking work" so they persist after PAW artifact stop-tracking.

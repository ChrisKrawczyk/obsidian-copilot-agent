...
- **Built‑in tool surface.** From the repo FAQ: *"By default, the SDK exposes the Copilot CLI's first‑party tools, similar to running the CLI with `--allow-all`."* This includes `read_file`, `edit_file`, `view`, shell tools, etc. **For v0.1 the design keeps built‑ins enabled but gates every call through `onPermissionRequest`**; the safety policy's universal-approval-gate routes built-in calls (host filesystem, shell, web-fetch) through the same UI as custom tools. Where built-in names overlap with our custom vault-scoped tools (`read_file`, `edit_file`), we shadow them with `overridesBuiltInTool: true`. Tool availability options remain configurable via session/client options if a future need to fully disable a class arises.
...
**Medium** for: confirmation that `onPermissionRequest` fires for every built‑in tool invocation (architectural premise of the universal-approval-gate; Phase 1 must verify with a synthetic shell call).
...
- **Whether `onPermissionRequest` fires for every built‑in invocation (shell, host‑filesystem, web‑fetch).** This is the architectural premise of the universal-approval-gate: built-ins are gated, not disabled. We override overlapping FS built-ins (`read_file`, `edit_file`) with `overridesBuiltInTool: true` so the model can't reach an unconfined host-FS path while a vault-scoped tool exists. Phase 1 spikes a synthetic shell call to verify the callback fires.
- **MCP integration story.** The SDK's `request.kind` enum includes `"mcp"`, suggesting first-class permission gating for MCP-bridged tools. Whether the SDK natively manages MCP server processes (à la Claude Desktop / VS Code) or expects the host to spawn the server and bridge tools via `defineTool` is not yet established. Phase 1 will inspect SDK source for an `mcpServers` config option; if absent, Phase 8 implements a minimal JSON-RPC client (initialize / tools/list / tools/call) and bridges discovered tools as `defineTool(\`<server>__<tool>\`)` registrations.
...
1. **Does `onPermissionRequest` fire for every built‑in tool invocation (shell, host‑filesystem, web‑fetch)?** This is the universal-approval-gate's architectural premise — if any built-in slips past the callback, the safety model is breached. → Next step: in the Phase 1 spike, send a prompt that should trigger a built-in shell call and observe whether `onPermissionRequest` fires before execution. Also inspect `nodejs/src/` for the request kinds the callback receives.
...
4. **Does the SDK natively manage MCP server processes, or do we bridge ourselves?** → Next step: inspect `dist/index.d.ts` for `mcpServers` / `mcpConfig` options on `CopilotClient` / `createSession`. If none, Phase 8 builds a JSON-RPC bridge.
5. **Exact `resumeSession(sessionId)` signature and failure modes.** → Next step: locate in `dist/index.d.ts`; observe what it returns when the session-state directory is corrupted or missing.
...
| **Built‑in Copilot CLI tools (shell, host filesystem read/write, web-fetch) are enabled by default.** | High blast radius if `onPermissionRequest` doesn't fire for them. | Phase 1 must verify the callback fires for every built-in invocation (synthetic shell call). Overlapping FS tools (`read_file`, `edit_file`) are shadowed with `overridesBuiltInTool: true` to keep the model on vault-confined paths. Non-overlapping built-ins (shell, web-fetch) are gated through the same approval pipeline. |
...
8. Verify `onPermissionRequest` fires for built-in tool invocations: prompt the model with a request that should trigger a built-in `shell` (or host-FS) call; confirm the callback fires with `request.kind === "shell"` before the tool executes.
9. Locate `resumeSession(sessionId)` in `dist/index.d.ts`; record signature and failure modes.
10. Inspect `CopilotClient` / `createSession` types for native MCP options (`mcpServers`, etc.); if absent, Phase 8 plans the JSON-RPC bridge.

Only if 1–10 pass, proceed to building Device Flow, the chat ItemView, the tool surface, and the safety policy.

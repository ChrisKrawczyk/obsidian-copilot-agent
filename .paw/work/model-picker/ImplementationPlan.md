...
- **Undo journal interaction (explicit guard).** `modelId` is per-conversation metadata, NOT transcript state — it MUST NOT be included in `UndoEntry` snapshots; an undo that restores transcript state preserves the currently-bound `modelId`. This pins the Spec edge case at `Spec.md:171` ("Undo journal interaction") in Phase 1, before the swap path exists in Phase 3.
...
- `ConversationManager.test.ts` (Undo + modelId guard): seed a conversation, snapshot via the Undo journal, call `setConversationModelId()` to change the bound model, then undo; assert that the post-undo `modelId` equals the post-swap value (NOT the snapshotted value) — i.e., `UndoEntry` does not carry `modelId` and undo cannot revert a model swap.
...
- **`src/sdk/ModelCatalog.ts`** (new): Session-scoped wrapper around `client.listModels()`. Public surface:
  ...
  - Pure helper `filterChatCapable(models: ModelInfo[]): ModelInfo[]`. **Concrete exclusion rule for FR-012** (per CodeResearch §1, the SDK exposes no public chat/embedding/image discriminator, so the only hard exclusions are the explicit-disabled signals; everything else passes through, true fail-open per Spec FR-012):
    1. **HARD exclusions (only):**
       - Drop models where `policy.state === "disabled"`.
       - Drop models where the explicit `disabled === true` boolean field is set on the SDK record.
    2. **SOFT signal (logged, NOT excluded):** ids matching the case-insensitive regex `embedding|image|dall-e|whisper|tts` are LOGGED via `console.warn` (telemetry / dogfooding signal — flags likely-non-chat ids that might benefit from a future curated allowlist) but **MUST NOT** be filtered out. A fabricated chat model whose id happens to contain such a substring (e.g., `gpt-image-reasoning`) MUST pass through to the picker.
    3. Everything else passes through unchanged. A family-prefix list (`gpt-`, `claude-`, `gemini-`, `o1-`, `o3-`, etc.) MAY be used as a positive signal for sort-order/scoring, but it is **never** an exclusion gate.
...
- **`SettingsTab.ts`**: Add a "Default model for new conversations" row using `addDropdown` (existing pattern at `:76-143`). Populate from `ModelCatalog.getState()`:
  ...
  - **Auto sentinel availability.** The `null` (Auto) sentinel bypasses availability checks and never triggers the unavailable Notice — `null` is always a valid selection because it defers to the onload heuristic at conversation-creation time.
- **`main.ts`**: Construct a **shared `CopilotClient`** at plugin onload (file-scope singleton, scoped to the plugin instance) used for catalog refresh and Settings UI; this is independent of the per-conversation runtime clients produced by the runtime factory (which continue to create per-conversation clients for `AgentSession`). The shared client MUST be constructed once during plugin init **before** `catalog.refresh()` is called. Construct `ModelCatalog` against the shared client, invoke `catalog.refresh()` once, and call `catalog.refresh()` after each token rotation (entitlements may change).

### Tests

- `ModelCatalog.test.ts` (new): all four state transitions; `filterChatCapable` HARD-excludes `policy.state === "disabled"` AND `disabled === true`; **soft-signal regression guard**: a stub record `{id: "gpt-image-reasoning"}` (a fabricated chat model whose id contains the soft-signal substring `image`) MUST pass through — this is the regression guard against the previous hard-exclusion gate; an ambiguous id `{id: "some-future-frobnicator"}` MUST also pass through (true fail-open); a chat record `{id: "gpt-4o"}` MUST pass; a `policy.state === "disabled"` record MUST be dropped; a `disabled: true` record MUST be dropped; ids matching the soft regex (e.g., `text-embedding-ada-002`, `dall-e-3`, `whisper-1`, `tts-1`) MUST be logged via `console.warn` (assert spy) but MUST still appear in the returned list. Empty list → `empty`; thrown error → `error`; subscribe/notify; retry-after-failure repopulates without re-construct.
...
- **Risk**: family-prefix list is mistaken for an exclusion gate, breaking FR-012. **Mitigation**: explicit unit fixture asserting an unknown-family id passes through; inline comment forbidding negative use; the only HARD negative gates are `policy.state === "disabled"` and `disabled === true`. The non-chat keyword regex (`embedding|image|dall-e|whisper|tts`) is a SOFT signal — logged via `console.warn` for telemetry only — and MUST NOT exclude. A regression-guard fixture pins a fabricated `gpt-image-reasoning` id passing through.
...
- **`AgentSession.ts`** — add `async swapModel(newModelId: string): Promise<void>`:
  ...
  2. If a turn is streaming, **mirror `ChatView.handleStop()` behavior** (`src/ui/ChatView.ts:492-519`): set the streaming placeholder's status to `interrupted` BEFORE calling `cancelCurrent()`, so the SDK abort is bucketed as a clean interruption rather than as `error` by the finalization path at `src/ui/ChatView.ts:727-805`. Then call the existing `cancelCurrent()` path (`:746-754`) and await stream cleanup.
...
- **`main.ts`**: Inject `ModelCatalog` and `SafetySettingsStore` into the runtime factory. When constructing each `AgentSession`, pass the persisted `modelId` as `preferredModel` so the SDK session is created bound to the correct model — this is the Phase 3 call site that satisfies SC-002 ("reload preserves bound model") end-to-end (Phase 1 covers the storage round-trip; Phase 3 covers the runtime construction passing the persisted id to `AgentSession` at session creation). `liveRuntimes` invariants — one entry per conversation, no leak across swaps — must hold across the new path.
...
- `AgentSession.test.ts` (interrupted finalization): when `swapModel()` is invoked during an in-flight stream, the streaming placeholder is finalized as `interrupted` (NOT `error`) — assert the bucketing matches `ChatView.handleStop()` behavior at `src/ui/ChatView.ts:492-519`, i.e., the placeholder's status is set to `interrupted` BEFORE the SDK abort surfaces.
...
- `modelPickerLogic.test.ts` (NFR-002 structural assertion): assert as a structural property test that the active-conversation switch path (the reducer transition from `active = A` to `active = B`) is purely synchronous — no `await`, no Promise return, no I/O surface. Implemented as a node-side assertion that the relevant function's `.constructor.name === "Function"` (not `"AsyncFunction"`) and that calling it returns a plain `PickerViewModel`, not a thenable. This pins the NFR-002 ≤16ms budget structurally rather than via wall-clock measurement.
...
- `ChatView.test.ts` (cancel-during-stream): when the picker is opened and a different model selected during a streaming turn, then the user cancels the confirmation dialog, the stream MUST continue to completion uninterrupted AND the placeholder MUST finalize as `complete` (NOT `interrupted` and NOT `error`). Pins Spec edge case `Spec.md:168` ("Cancel during stream: Cancelling the confirmation dialog while a turn is streaming leaves the streaming turn untouched; only confirmation triggers interruption").
...
| SC-002 | Reload preserves bound model | 1 + 3 | Phase 1: persistence round-trip. Phase 3: runtime construction passes persisted `modelId` to `AgentSession` at session creation. |
...
| Undo across restart | `UndoJournal` round-trip tests + manual smoke from v0.3 plan; **Phase 1 adds an explicit Undo+modelId guard test** asserting `UndoEntry` does not carry `modelId` and undo cannot revert a model swap. | Phase 1 (persisted shape change; explicit Undo+modelId guard), Phase 3 (runtime construction order) |
...
### Holistic Review Revision Notes

Edits applied from the holistic planning-docs review (consolidated from gpt-5.4, claude-opus-4.7, gemini-3.1-pro-preview); 13 fixes total across plan and spec. The 8 plan-targeted fixes are listed below; the 5 spec-targeted fixes are recorded in `Spec.md`'s Revision Notes section.

- **Gemini-PDR-1 (must-fix) — Shared CopilotClient ownership for global catalog.** Phase 2 Implementation now spells out that `main.ts` constructs a shared `CopilotClient` at plugin onload, scoped to the plugin instance and used by `ModelCatalog` and the Settings UI for catalog refresh. This shared client is independent of the per-conversation runtime clients produced by the runtime factory, and it MUST be constructed once during plugin init before `catalog.refresh()` is invoked.
- **GPT-PDR-1 / Opus-PDR-1 (must-fix) — FR-012 substring filtering downgraded to soft signal.** Rewrote `filterChatCapable()` so the only HARD exclusions are `policy.state === "disabled"` and `disabled === true`; the substring regex (`embedding|image|dall-e|whisper|tts`) is now logged via `console.warn` for telemetry but does NOT exclude. Tests rewritten: assert disabled exclusion; assert ambiguous-id passthrough; assert that a fabricated chat model with id `gpt-image-reasoning` PASSES through (regression guard against the previous gate); previously-asserted hard-exclusion fixtures for `text-embedding-ada-002`, `dall-e-3`, `whisper-1`, `tts-1` reframed as soft-signal log assertions while the records still appear in the returned list. Phase 2 risk wording updated to match.
- **Opus-PDR-2 / GPT-PDR-2 (must-fix) — Undo + modelId explicit guard.** Phase 1 Implementation now states that `modelId` is per-conversation metadata and MUST NOT be included in `UndoEntry` snapshots; an undo restoring transcript state preserves the currently-bound `modelId`. Companion test added in `ConversationManager.test.ts`: snapshot, change `modelId` via `setConversationModelId()`, undo, assert post-undo `modelId` equals the post-swap value (not the snapshotted value). NFR-005 phase-local regression matrix Undo row updated to mention the Phase 1 explicit guard.
- **Opus-PDR-4 (should-fix) — Cancel-during-stream test.** Added Phase 4 test asserting that opening the picker, selecting a different model during a streaming turn, then cancelling the confirmation dialog leaves the stream uninterrupted AND the placeholder finalizes as `complete` (not `interrupted` and not `error`). Pins Spec edge case `Spec.md:168`.
- **Gemini-PDR-2 (should-fix) — Stream-interrupt finalization order.** Phase 3 `swapModel()` orchestration now mirrors `ChatView.handleStop()`: the streaming placeholder's status is set to `interrupted` BEFORE calling `cancelCurrent()`, so the SDK abort is bucketed cleanly as `interrupted` rather than `error` by the finalization path at `src/ui/ChatView.ts:727-805`. Companion Phase 3 test added asserting the placeholder is finalized as `interrupted` (not `error`) when swap interrupts a stream.
- **GPT-PDR-3 (should-fix) — SC-002 traceability.** Updated FR/NFR/SC coverage matrix: SC-002 now maps to "Phase 1 + Phase 3" (Phase 1 covers persistence round-trip; Phase 3 covers runtime construction passing persisted `modelId` to `AgentSession` at session creation). Phase 3 `main.ts` Implementation note expanded to surface this end-to-end path.
- **Opus-PDR-7 (should-fix) — NFR-002 structural assertion.** Added `modelPickerLogic.test.ts` test asserting as a structural property that the active-conversation switch path is purely synchronous (no `await`, no `AsyncFunction`, no thenable return). This pins the NFR-002 ≤16ms budget structurally rather than via wall-clock measurement.
- **Gemini-PDR-3 (nit) — Auto sentinel availability.** Phase 2 Settings implementation now notes explicitly: the `null` (Auto) sentinel bypasses availability checks and never triggers the unavailable Notice — `null` is always a valid selection because it defers to the onload heuristic at conversation-creation time.

Cross-artifact sanity check: Spec `Out of Scope (deferred)` additions (reasoning-effort metadata in picker rows; per-conversation reasoning-effort controls; soft warning when switching to a smaller-context model; telemetry on per-conversation model usage; cross-session pricing/policy display) all match existing entries in the plan's `What We're NOT Doing` and `Phase Candidates` sections — no new conflicts introduced.
